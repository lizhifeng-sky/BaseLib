...
import lzf.music.FloatWindow;
import lzf.music.R;
import lzf.music.util.ProgressTextUtils;
...
public class MusicService extends Service implements PlayerInterface, View.OnClickListener, SeekBar.OnSeekBarChangeListener, MediaPlayer.OnCompletionListener, MediaPlayer.OnPreparedListener, MediaPlayer.OnBufferingUpdateListener {
    ...
    @Override
    public void start() {
        if(audioState == AudioState.STATE_PAUSE || audioState == AudioState.STATE_READY) {
            startAnimator().start();
        } else {
            audioStart();
        }
    ...
    public void previous() {
        if(animatorState == AnimatorState.STATE_END && audioState != AudioState.STATE_PREPARE) {
    ...
    private void audioPrepare() {
        try {
            ...
            setAudioState(AudioState.STATE_PREPARE);
    ...
    public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
        if(fromUser && mediaPlayer!=null && touchState != AudioState.STATE_STOP) {
